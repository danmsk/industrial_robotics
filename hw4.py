...
def get_single_image(topic_name='/camera/image_raw', timeout=5.0):
    bridge = CvBridge()
    try:
        # Подождать первое сообщение с изображением (timeout в секундах)
        msg = rospy.wait_for_message(topic_name, Image, timeout=timeout)
        # Конвертировать в OpenCV изображение
        cv_image = bridge.imgmsg_to_cv2(msg, desired_encoding='bgr8')
        return cv_image
    except rospy.ROSException as e:
        rospy.logerr("Не удалось получить изображение: %s", e)
        return None
...
def create_transformation_matrix(R, translation):
    """
    Создаёт 4x4 матрицу однородного преобразования из
    матрицы поворота R (3x3) и вектора трансляции translation (3,).

    Args:
        R: numpy.ndarray, форма (3,3) — матрица поворота
        translation: numpy.ndarray или список, длина 3 — вектор трансляции

    Returns:
        T: numpy.ndarray, форма (4,4) — матрица преобразования
    """
    T = np.eye(4)
    T[:3, :3] = R
    T[:3, 3] = translation
...
def get_marker_coordinates(image):
        # Параметры камеры и маркеров (скопированы из твоего кода)
    ...
    dist_coeffs = np.array([-0.046462, 0.284386, 0.003340, -0.001596, 0.0])

    main_marker_id = 3
    main_marker_size = 0.10      # 10 см
    other_marker_size = 0.04     # 4 см

    base_to_main_marker_translation = np.array([-0.162, -0.274, 0.0025])
    R_b_mm = np.eye(3)
    base_to_main_marker = create_transformation_matrix(R_b_mm, base_to_main_marker_translation)
    ...
    """
    Принимает BGR изображение (numpy.ndarray),
    ищет ArUco маркеры,
    возвращает словарь:
      {
        marker_id: {
    ...
    Если маркеры не найдены — возвращает пустой словарь.
    """
    ...
    for i, marker_id in enumerate(ids):
        size = main_marker_size if marker_id == main_marker_id else other_marker_size
    ...
    main_indices = [i for i, mid in enumerate(ids) if mid == main_marker_id]
    if len(main_indices) != 1:
        # Если основной маркер найден неправильно — возвращаем пустой результат
        return {}
    ...
    main_marker_to_camera = create_transformation_matrix(R, tvecs[main_index].flatten())
    base_to_camera = main_marker_to_camera @ invert_transform(base_to_main_marker)
    camera_to_base = invert_transform(base_to_camera)
    ...
    for i, marker_id in enumerate(ids):
        ...
        marker_to_camera = create_transformation_matrix(R_m, tvecs[i].flatten())
        pos_camera = marker_to_camera[:3, 3]

        marker_to_base = camera_to_base @ marker_to_camera
        pos_base = marker_to_base[:3, 3]



        results[marker_id] = {
            'camera_coords': pos_camera,
            'base_coords': pos_base
        }
...
def main():
    ...
    try:
        ...
        img = get_single_image('/pylon_camera_node/image_raw')
        if img is not None:
            rospy.loginfo("Изображение получено, размер: %dx%d" % (img.shape[1], img.shape[0]))
        # Дальше можно обработать или сохранить
        else:
            rospy.logwarn("Изображение не получено")
        
        # Переход в тоску над конвейером
        move_group.go_conveyor_up()


        # Выбор маркера и получение координат
        marker_id_of_interest = 8
        coordinates = get_marker_coordinates(img)
        base_coords = coordinates[marker_id_of_interest]['base_coords']
        x = base_coords[0]
        y = base_coords[1]
        z = base_coords[2]
        print(f"x = {x}, y = {y}, z = {z}")
